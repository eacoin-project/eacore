EACoin Core version 0.11.0 is now available from:

  <https://eacoin.org/bin/eacoin-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of EACoin Core or other software:
...
For example, add the following to `eacoin.conf`:
...
related to the blockchain in the eacoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows EACoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that EACoin Core will 
...
For an example script see [eacoin-submittx](https://github.com/laanwj/eacoin-submittx).
...
- #5334 `cf87536` libeacoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new eacoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only EACoinAmountField
- #5753 `0f44672` Add eacoin logo to about screen
...
- #5809 `46bfbe7` Add eacoin-cli man page
...
- #5849 `41259ca` contrib/init/eacoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename EACoin-Qt.app to EACoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for eacoin-tx)

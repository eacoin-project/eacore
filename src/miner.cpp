...
// ***TODO*** ScanHash is not yet used in EACoin
//
...
void static BitcoinMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("EACoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("EACoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
